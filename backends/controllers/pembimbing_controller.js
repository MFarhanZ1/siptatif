    ...
    try {
		...
		if (search) {
			...
			results = await db.query(
				"SELECT vd.nidn, vd.nama, vd.keahlian, dp.kuota FROM view_detail_dosen vd, dosen_pembimbing dp WHERE (LOWER(vd.nama) LIKE $1 OR LOWER(vd.nidn) LIKE $1 OR LOWER(vd.keahlian) LIKE $1) AND dp.nidn = vd.nidn LIMIT $2 OFFSET $3",
				[regexSearch, limit, offset]
			);
			countData = await db.query(
				"SELECT COUNT(*) FROM view_detail_dosen vd, dosen_pembimbing dp WHERE (LOWER(vd.nama) LIKE $1 OR LOWER(vd.nidn) LIKE $1 OR LOWER(vd.keahlian) LIKE $1) AND dp.nidn = vd.nidn",
				[regexSearch]
			);
		}
		...
		// bakal query execute mode paging
			...
			results = await db.query(
				"SELECT vd.nidn, vd.nama, vd.keahlian, dp.kuota FROM view_detail_dosen vd, dosen_pembimbing dp WHERE dp.nidn = vd.nidn LIMIT $1 OFFSET $2",
				[limit, offset]
			);
			countData = await db.query("SELECT COUNT(*) FROM view_detail_dosen vd, dosen_pembimbing dp WHERE dp.nidn = vd.nidn");
		}

		// bakal query execute mode normal, langsung terobos semua data jika tak ada query search/page
		else {
            results = await db.query("SELECT vd.nidn, vd.nama, vd.keahlian, dp.kuota from view_detail_dosen vd, dosen_pembimbing dp where dp.nidn = vd.nidn and dp.kuota > 0");
