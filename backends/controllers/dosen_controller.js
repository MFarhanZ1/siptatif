	...
	try {
		...
		if (search) {
			...
			results = await db.query(
				"SELECT * FROM dosen WHERE LOWER(nama) LIKE $1 OR LOWER(nidn) LIKE $1 ORDER BY nama LIMIT $2 OFFSET $3",
			...
			results = await db.query("SELECT * FROM dosen ORDER BY nama LIMIT $1 OFFSET $2", [
				limit,
				offset,
			]);
